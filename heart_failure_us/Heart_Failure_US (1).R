...
# Heart Failure Comorbidity HFrEF ------------------

HF_Demographics <- fread("HF Analysis Results 7.0/HF Demographics.txt")
names(HF_Demographics)[1] <- "patient"
sum(HF_Demographics$weight) # 17277771
HF_Demographics <- HF_Demographics %>% filter(died=="N" | death_date>"2020-05-01") %>% filter(heart_failure_onset >= "2016-05-01")
HF_Drug_Histories <- fread("HF Analysis Results 7.0/HF Drug Histories.txt", colClasses = "character")
HF_Drug_Histories <- gather(HF_Drug_Histories, Month, Drugs, month1:month60, factor_key=TRUE)
HF_Drug_Histories <- HF_Drug_Histories %>% filter(Drugs!="-" & Drugs != "104")
HF_Drug_Histories <- HF_Drug_Histories %>% select(patient) %>% distinct()
HF_Demographics %>% inner_join(HF_Drug_Histories) %>% summarise(n=sum(weight)) # 10149523
...
DANU_Diagnosis_Codes <- fread("HF Analysis Results 7.0/DANU Diagnosis Codes.txt")

DANU_Diagnosis_Codes <- DANU_Diagnosis_Codes %>% filter(diagnosis == "Heart Failure")

DANU_Diagnosis_Codes <- DANU_Diagnosis_Codes %>% select(code, description)

DANU_Dossiers <- fread("DANU Demographics 7.0/DANU Dossiers Full.txt")
names(DANU_Dossiers)[1] <- "patient"
...
Systolic_Pats <- Systolic_Pats %>% left_join(HF_Demographics %>% select(patid, weight), by=c("patient"="patid"))
sum(Systolic_Pats$weight) # 1706563


DANU_Demographics <- fread("DANU Demographics 7.0/DANU Demographics Full.txt")
DANU_Demographics <- DANU_Demographics %>% select(patid, weight, diagnosis) %>% filter(grepl("Diabetes", diagnosis)|grepl("Obesity", diagnosis))
names(DANU_Demographics)[1] <- "patient"


Systolic_Pats <- Systolic_Pats %>% left_join(DANU_Demographics)



HF_Comorbidity_Inventories <- fread("HF Analysis Results 7.0/HF Comorbidity Inventories.txt", colClasses = "character")
names(HF_Comorbidity_Inventories)[1] <- "patient"

Comorb <- HF_Comorbidity_Inventories %>% filter(grepl("G45", diagnosis)|
                                         grepl("I63", diagnosis)|
                                         grepl("H34", diagnosis)|
                                         grepl("I65", diagnosis)|
                                         grepl("I66", diagnosis)|
                                         ...
                                         grepl("I25", diagnosis)|
                                         grepl("I70", diagnosis)|
                                         grepl("I73", diagnosis)) %>% select(patient) %>% distinct()

Systolic_Pats <- Systolic_Pats %>% left_join(Comorb %>% mutate(ASCVD="ASCVD"))



DANU_Events <- fread("DANU Demographics 7.0/DANU Events Full.txt")
DANU_Events <- DANU_Events %>% filter(grepl("BMI", code ))
DANU_Events$code <- parse_number(DANU_Events$code)
DANU_Events <- DANU_Events %>% group_by(patid) %>% filter(code == max(code)) %>% slice(1)


DANU_Events <- DANU_Events %>% ungroup() %>% select(patid, code) %>% mutate(code=ifelse(code>30, ">30",
                                                                          ifelse(code>27,">27", "none"))) %>% select(patid, code) %>% distinct()
names(DANU_Events)[1] <- "patient"


Systolic_Pats <- Systolic_Pats %>% left_join(DANU_Events)

Systolic_Pats %>% filter(grepl("Diabetes", diagnosis)) %>% summarise(n=sum(weight)/1706563)
Systolic_Pats %>% filter(grepl("Obesity", diagnosis)) %>% summarise(n=sum(weight)/1706563)
Systolic_Pats %>% filter(ASCVD=="ASCVD") %>% summarise(n=sum(weight)/1706563)
Systolic_Pats %>% filter(!is.na(code)) %>% summarise(n=sum(weight))
Systolic_Pats %>% filter(ASCVD=="ASCVD"&code %in% c(">30", ">27"))  %>% summarise(n=sum(weight)/1102007)
Systolic_Pats %>% filter(code %in% c(">30"))  %>% summarise(n=sum(weight)/1102007)



Systolic_Pats %>% mutate(OBE=ifelse(ASCVD=="ASCVD"&code %in% c(">30", ">27"),1,0)) %>%
  mutate(DIA=ifelse(grepl("Diabetes", diagnosis),1,0)) %>%
  filter(!is.na(code)) %>% group_by(OBE, DIA) %>% summarise(n=sum(weight)/1102007)
