...
# GERD Sizes Segments Dx | Rx -------------------- 


# Define ranges
gerd_ranges <- tibble(
  Group = c("[1] Syptomatic GERD", "[2] Diagnosed GERD", "[3] Treated GERD", "[6] Rx + OTC", "[5] Rx-only", "[4] OTC-only"),
  Min = c(50, 30, 15, 3, 7,  5),
  Max = c(80, 40, 25, 5, 10,  10)
)


# Plot
plot <- ggplot(gerd_ranges) +
  geom_segment(aes(y = Group, yend = Group,x = Min, xend = Max),size = 6, lineend = "round", color = "#8499b1") +
  scale_x_continuous(limits=c(0,100)) +
  # Add Min labels (left side)
  geom_text(aes(
    x = Min-1,
    y = Group,
    label = paste0(Min, "M")
  ),
  hjust = 1.3,  # pushes text slightly left of segment
  size = 4
  ) +
  # Add Max labels (right side)
  geom_text(aes(
    x = Max+1,
    y = Group,
    label = paste0(Max, "M")
  ),
  hjust = -0.3,  # pushes text slightly right of segment
  size = 4
  ) +
  labs(title = "Estimated GERD Segment Sizes \n", x = "\nGERD Size [Million Patients]", y = "") +
  theme_minimal() +
...
ggsave(file="../out/gerd_prev_groups.svg", plot=plot, width=7, height=3)
...
df <- tibble(
  drug_ahfs_class = c(
    ...
    "Other Antiacid / Antiulcer",
    "Proton-Pump Inhibitors"
  ),
  perc = c(0.983, 0.984, 0.966)
)

# Create complement and reshape
df_long <- df %>%
  mutate(complement = 1 - perc) %>%
  pivot_longer(
    cols = c(perc, complement),
    names_to = "type",
    values_to = "value"
  )
...
# Plot
plot <- df_long %>%
  mutate(drug_ahfs_class=ifelse(drug_ahfs_class=="Antihistamines (GI Drugs)", "Antihistamine",
                                ifelse(drug_ahfs_class=="Other Antiacid / Antiulcer", "Antiacid", "PPI"))) %>%
  ggplot(aes(x = drug_ahfs_class, y = value, fill = type)) +
  geom_col(width = 0.7) +
  scale_y_continuous(labels = percent_format(accuracy = 1)) +
  scale_fill_manual(
    values = c("perc" = "#8499b1", "complement" = "#ecc6d2"),
    labels = c("Brand", "Generic")
  ) +
  labs(
    title = "Drug Class Generic % Share",
    x = "",
    y = "Patient Scripts Volume %",
    fill = ""
  ) +
...
ggsave(file="../out/generic_share.svg", plot=plot, width=5, height=5)
...
# Data
gerd_buckets <- tibble(
  Class = c("Antihistamines (GI Drugs",
                 "Other Antiacid | Antiulcer",
                 "Proton-Pump Inhibitors"),
  Percent = c(34, 10, 56)
)
...
# Keep intended order
gerd_buckets$Class <- factor(
  gerd_buckets$Class,
  levels = gerd_buckets$Class
)
...
plot <- ggplot(gerd_buckets,
  ...
  labs(title = "Scripts Volume Share by Drug Class") +
  theme_void() +
  theme(
    text = element_text(face = "bold"),
    plot.title = element_text(hjust = 0.5, size = 14)
...
ggsave(file = "../out/drug_class_share.svg", plot = plot, width = 6, height = 5)
