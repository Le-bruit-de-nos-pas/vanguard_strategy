...
# KIm Lie Oct 10 2025 -------
New_Primary_Cancer_Box <- fread("New_Primary_Cancer_Box.txt", sep="\t")
New_Primary_Cancer_Box <- New_Primary_Cancer_Box %>% filter(Primary_Cancer %in% c("Lung Cancer", "Intestinal Cancer", "Pancreatic Cancer"))
New_Primary_Cancer_Box <- New_Primary_Cancer_Box %>% select(patid, Primary_Cancer, weight)

# Death/Survival
PONS_Demographics_temp <- fread("PONS Demographics.txt")
PONS_Demographics_temp <- PONS_Demographics_temp %>% filter(!is.na(cancer_metastasis)) %>%
  select(patid, weight, cancer_onset, death_date)
...
missingDeathDay <- ymd("2021-08-01")
PONS_Demographics_temp <- PONS_Demographics_temp %>% mutate(death_date = case_when(is.na(death_date) ~ missingDeathDay, TRUE ~ death_date))

PONS_Demographics_temp <- PONS_Demographics_temp %>% mutate(Survived = as.numeric(death_date)-as.numeric(cancer_onset)) %>%
  mutate(Survived= round(Survived / 30.5,0)) %>%
  mutate(censor=ifelse(death_date=="2021-08-01",0,1))

New_Primary_Cancer_Box <- PONS_Demographics_temp %>% inner_join(New_Primary_Cancer_Box)


New_Primary_Cancer_Box %>% inner_join(PONS_Demographics_temp %>% select(patid)) %>%
  group_by(Primary_Cancer) %>% summarise(mean=mean(Survived))


#   Primary_Cancer     mean
#   <chr>             <dbl>
# 1 Intestinal Cancer  38.0
# 2 Lung Cancer        31.3
# 3 Pancreatic Cancer  23.4

library(survival)
library(ggsurvfit)

s1 <- survival::survfit(survival::Surv(Survived, censor) ~ Primary_Cancer, data = New_Primary_Cancer_Box)
...
survfit2(Surv(Survived, censor) ~ Primary_Cancer, data = New_Primary_Cancer_Box) %>% 
  ggsurvfit() +
...
CancerDrug_Experienced <- fread("CancerDrug_Experienced.txt", sep="\t")
CAN_Drug_Histories <- CAN_Drug_Histories %>%  inner_join(CancerDrug_Experienced, by=c("patient"="patid"))
CAN_Drug_Histories <- New_Primary_Cancer_Box %>% select(patid) %>% distinct() %>% inner_join(CAN_Drug_Histories, by=c("patid"="patient"))
CAN_Drug_Histories <- gather(CAN_Drug_Histories, Month, Treat, month1:month60, factor_key=TRUE)
...
# PD-1/PD-L1 w/chemo
# Targeted therapy (HER2, EGFR, ALK, ROS1, etc)
# Platinum-based chemo
# Non-platinum chemo

Lung_pats <- New_Primary_Cancer_Box %>% filter(Primary_Cancer=="Lung Cancer") %>%
  select(patid, Survived, censor)  

PD1_Chemo_pats <- Lung_pats %>% inner_join(CAN_Drug_Histories %>% filter(chemo_class %in% c("Alkylating", "Antimetabolites", "Antimicrotubule", "Other", "Topoisomerase"))) %>% select(-chemo_class) %>%
  inner_join(Lung_pats %>% inner_join(CAN_Drug_Histories %>% filter(chemo_class=="PD1/PDL1")) %>% select(-chemo_class)) %>% mutate(chemo_class="PD1/PDL1_wChemo")

Lung_pats <- Lung_pats %>% inner_join(CAN_Drug_Histories %>% filter(chemo_class=="PD1/PDL1")) %>%
  bind_rows(Lung_pats %>% inner_join(CAN_Drug_Histories %>% filter(chemo_class %in% c("Immuno/Targeted", "Biologic")) %>% mutate(chemo_class="Target") %>% distinct())) %>%
    bind_rows(Lung_pats %>% inner_join(CAN_Drug_Histories %>% filter(chemo_class=="Platinum agent"))) %>%
  bind_rows(Lung_pats %>% inner_join(CAN_Drug_Histories %>% filter(chemo_class %in% c("Alkylating", "Antimetabolites", "Antimicrotubule", "Other", "Topoisomerase")) %>% mutate(chemo_class="Non-platinum Chemo") %>% distinct())) %>%
  bind_rows(PD1_Chemo_pats)

Lung_pats %>% select(patid, chemo_class) %>% distinct() %>% mutate(exp=1) %>% spread(key=chemo_class, value=exp) %>%
  group_by(`Non-platinum Chemo`, `PD1/PDL1`, `PD1/PDL1_wChemo`, `Platinum agent`, Target) %>% count()

Lung_pats %>% select(patid, chemo_class) %>% distinct() %>% group_by(patid) %>% count() %>%
  ungroup() %>% group_by(n) %>% count() %>% ungroup() %>% mutate(perc=nn/sum(nn))


s1 <- survival::survfit(survival::Surv(Survived, censor) ~ chemo_class, data = Lung_pats)
...
survfit2(Surv(Survived, censor) ~ chemo_class, data = Lung_pats) %>% 
  ggsurvfit() +
  labs(x = "Months",y = "LUNG CANCER \n Overall survival probability") + 
  ylim(0,1) +
  theme_minimal() +
  add_confidence_interval() +
  ...
  ggtitle("LUNG CANCER \nKM Survial Curves per treatment class tried")
...
median_summary <- data.frame(
  ...
  median = c(39, 35, 34, 34, 50),
  lower = c(37, 34, 33, 33, 48),
  upper = c(41, 36, 36, 35, 52)
)



# 800 300
ggplot(median_summary, aes(
  x = median,
  y = reorder(chemo_class, median),
  xmin = lower,
  xmax = upper,
  color = chemo_class
)) +
  geom_point(size = 5, shape=1 , stroke=2) +
  geom_errorbarh(height = 0.5, size=2, alpha=0.5) +
  geom_text(
    aes(label = sprintf("%d (%.0f–%.0f)", median, lower, upper)),
    hjust = -0.1, size = 3.5
  ) +
  scale_color_manual(values = c(
    "Non-platinum Chemo" = "#27BEF5",
    "PD1/PDL1" = "#63851B",
    "PD1/PDL1_wChemo" = "#E6A810",
    "Platinum agent" = "#131B70",
    "Target" = "#F52727"
  )) +
  scale_x_continuous("\n Median survival from cancer onset (months)", limits = c(0, 60)) +
  theme_minimal(base_size = 13) +
  theme(
    axis.title.y = element_blank(),
    legend.position = "none"
  ) +
  ggtitle("LUNG CANCER \nMedian Overall Survival (95% CI) per treatment class tried")
...
Intestinal_pats <- New_Primary_Cancer_Box %>% filter(Primary_Cancer=="Intestinal Cancer") %>%
  select(patid, Survived, censor)  

PD1_Chemo_pats <- Intestinal_pats %>% inner_join(CAN_Drug_Histories %>% filter(chemo_class %in% c("Alkylating", "Antimetabolites", "Antimicrotubule", "Other", "Topoisomerase"))) %>% select(-chemo_class) %>%
  inner_join(Intestinal_pats %>% inner_join(CAN_Drug_Histories %>% filter(chemo_class=="PD1/PDL1")) %>% select(-chemo_class)) %>% mutate(chemo_class="PD1/PDL1_wChemo")

Intestinal_pats <- Intestinal_pats %>% inner_join(CAN_Drug_Histories %>% filter(chemo_class=="PD1/PDL1")) %>%
  bind_rows(Intestinal_pats %>% inner_join(CAN_Drug_Histories %>% filter(chemo_class %in% c("Immuno/Targeted", "Biologic")) %>% mutate(chemo_class="Target") %>% distinct())) %>%
    bind_rows(Intestinal_pats %>% inner_join(CAN_Drug_Histories %>% filter(chemo_class=="Platinum agent"))) %>%
  bind_rows(Intestinal_pats %>% inner_join(CAN_Drug_Histories %>% filter(chemo_class %in% c("Alkylating", "Antimetabolites", "Antimicrotubule", "Other", "Topoisomerase")) %>% mutate(chemo_class="Non-platinum Chemo") %>% distinct())) %>%
  bind_rows(PD1_Chemo_pats)



Intestinal_pats %>% select(patid, chemo_class) %>% distinct() %>% mutate(exp=1) %>% spread(key=chemo_class, value=exp) %>%
  group_by(`Non-platinum Chemo`, `PD1/PDL1`, `PD1/PDL1_wChemo`, `Platinum agent`, Target) %>% count()

Intestinal_pats %>% select(patid, chemo_class) %>% distinct() %>% group_by(patid) %>% count() %>%
  ungroup() %>% group_by(n) %>% count() %>% ungroup() %>% mutate(perc=nn/sum(nn))
...
s1 <- survival::survfit(survival::Surv(Survived, censor) ~ chemo_class, data = Intestinal_pats)
...
survfit2(Surv(Survived, censor) ~ chemo_class, data = Intestinal_pats) %>% 
  ggsurvfit() +
  labs(x = "Months",y = "Intestinal CANCER \n Overall survival probability") + 
  ylim(0,1) +
  theme_minimal() +
  add_confidence_interval() +
  add_risktable(risktable_height=0.3) +
...
Pancreatic_pats <- New_Primary_Cancer_Box %>% filter(Primary_Cancer=="Pancreatic Cancer") %>%
  select(patid, Survived, censor)  

PD1_Chemo_pats <- Pancreatic_pats %>% inner_join(CAN_Drug_Histories %>% filter(chemo_class %in% c("Alkylating", "Antimetabolites", "Antimicrotubule", "Other", "Topoisomerase"))) %>% select(-chemo_class) %>%
  inner_join(Pancreatic_pats %>% inner_join(CAN_Drug_Histories %>% filter(chemo_class=="PD1/PDL1")) %>% select(-chemo_class)) %>% mutate(chemo_class="PD1/PDL1_wChemo")

Pancreatic_pats <- Pancreatic_pats %>% inner_join(CAN_Drug_Histories %>% filter(chemo_class=="PD1/PDL1")) %>%
  bind_rows(Pancreatic_pats %>% inner_join(CAN_Drug_Histories %>% filter(chemo_class %in% c("Immuno/Targeted", "Biologic")) %>% mutate(chemo_class="Target") %>% distinct())) %>%
    bind_rows(Pancreatic_pats %>% inner_join(CAN_Drug_Histories %>% filter(chemo_class=="Platinum agent"))) %>%
  bind_rows(Pancreatic_pats %>% inner_join(CAN_Drug_Histories %>% filter(chemo_class %in% c("Alkylating", "Antimetabolites", "Antimicrotubule", "Other", "Topoisomerase")) %>% mutate(chemo_class="Non-platinum Chemo") %>% distinct())) %>%
  bind_rows(PD1_Chemo_pats)




Pancreatic_pats %>% select(patid, chemo_class) %>% distinct() %>% mutate(exp=1) %>% spread(key=chemo_class, value=exp) %>%
  group_by(`Non-platinum Chemo`, `PD1/PDL1`, `PD1/PDL1_wChemo`, `Platinum agent`, Target) %>% count()

Pancreatic_pats %>% select(patid, chemo_class) %>% distinct() %>% group_by(patid) %>% count() %>%
  ungroup() %>% group_by(n) %>% count() %>% ungroup() %>% mutate(perc=nn/sum(nn))
...
s1 <- survival::survfit(survival::Surv(Survived, censor) ~ chemo_class, data = Pancreatic_pats)
...
survfit2(Surv(Survived, censor) ~ chemo_class, data = Pancreatic_pats) %>% 
  ggsurvfit() +
  labs(x = "Months",y = "Pancreatic CANCER \n Overall survival probability") + 
  ylim(0,1) +
  theme_minimal() +
  add_confidence_interval() +
  add_risktable(risktable_height=0.3) +
...
median_summary <- data.frame(
  ...
  median = c(21, 39, 40, 23, 38),
  lower = c(20, 30, 30, 21, 34),
  upper = c(22, 51, 50, 24, 47)
)
...
Pats_LIP_to_check <- Lung_pats %>% mutate(Cancer="LUNG") %>% 
  bind_rows(Intestinal_pats %>% mutate(Cancer="INTESTINAL")) %>%
  bind_rows(Pancreatic_pats %>% mutate(Cancer="PANCREATIC")) 
...
Pats_to_track_BMI <- Pats_to_track_BMI %>% inner_join(Pats_LIP_to_check %>% select(patid))
...
New_Cachexia_Pred_big <- temp %>% filter(Drop95==1 | Drop90==1 | Drop2_20==1) %>% select(patid) %>% distinct()
New_Cachexia_Pred_small <- temp %>% filter( Drop95==1 | Drop2_20==1) %>% select(patid) %>% distinct()
...
df <- Pats_LIP_to_check %>% inner_join(temp %>% select(patid) %>% distinct()) %>%
  group_by(chemo_class) %>% count() %>% rename("den"="n") %>%
  left_join(
    Pats_LIP_to_check %>% inner_join(New_Cachexia_Pred_big %>% select(patid) %>% distinct()) %>%
  group_by(chemo_class) %>% count() %>% rename("num_big"="n")
  ) %>%
   left_join(
    Pats_LIP_to_check %>% inner_join(New_Cachexia_Pred_small %>% select(patid) %>% distinct()) %>%
  group_by(chemo_class) %>% count() %>% rename("num_small"="n")
  ) %>%
  mutate(perc_big=num_big/den, perc_small=num_small/den) %>% ungroup()
...
df_long <- df %>%
  select(Cancer, perc_big, perc_small) %>%
  pivot_longer(cols = c(perc_big, perc_small), 
               names_to = "Type", values_to = "Percentage") %>%
  mutate(Type = recode(Type, perc_big = "With 10% 12month Drop", perc_small = "Exc. 10% Drops"))

# Plot side-by-side bar charts
ggplot(df_long, aes(x = Cancer, y = Percentage, fill = Type)) +
  geom_col(position = position_dodge(width = 0.7), width = 0.6) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  scale_fill_manual(values = c("Big" = "#1b9e77", "Small" = "#d95f02")) +
  labs(y = "Percentage", x = "Cancer Type", fill = "Type") +
  theme_minimal(base_size = 13) +
  ggtitle("CachexiaPercentage of Big and Small by Cancer Type") +
...
Pats_LIP_to_check %>% inner_join(temp %>% select(patid) %>% distinct()) %>%
  group_by(Cancer, chemo_class) %>% count() %>% rename("den"="n") %>%
  left_join(
    Pats_LIP_to_check %>% inner_join(New_Cachexia_Pred_big %>% select(patid) %>% distinct()) %>%
  group_by(Cancer, chemo_class) %>% count() %>% rename("num_big"="n")
  ) %>%
   left_join(
    Pats_LIP_to_check %>% inner_join(New_Cachexia_Pred_small %>% select(patid) %>% distinct()) %>%
  group_by(Cancer, chemo_class) %>% count() %>% rename("num_small"="n")
  ) %>%
  mutate(perc_big=num_big/den, perc_small=num_small/den) %>% ungroup() %>%
  group_by(Cancer) 
...
# KIm Lie Oct 10 2025 cachexia dx  -------
New_Primary_Cancer_Box <- fread("New_Primary_Cancer_Box.txt", sep="\t")
New_Primary_Cancer_Box <- New_Primary_Cancer_Box %>% filter(Primary_Cancer %in% c("Lung Cancer", "Intestinal Cancer", "Pancreatic Cancer"))
New_Primary_Cancer_Box <- New_Primary_Cancer_Box %>% select(patid, Primary_Cancer, weight)
...
# Death/Survival
PONS_Demographics_temp <- fread("PONS Demographics.txt")
PONS_Demographics_temp <- PONS_Demographics_temp %>% select(patid, weight, cancer_onset, cachexia_onset)
names(PONS_Demographics_temp)
PONS_Demographics_temp$cancer_onset <- as.Date(PONS_Demographics_temp$cancer_onset)
PONS_Demographics_temp$cachexia_onset    <- as.Date(PONS_Demographics_temp$cachexia_onset)
PONS_Demographics_temp <- PONS_Demographics_temp %>% drop_na() 

PONS_Demographics_mets_pats <- fread("PONS Demographics.txt")
PONS_Demographics_mets_pats <- PONS_Demographics_mets_pats %>% filter(!is.na(cancer_metastasis)) %>% select(patid)
...
CAN_Drug_Histories <- CAN_Drug_Histories %>% select(-disease,-Month) %>% distinct()
...
Lung_pats <- Lung_pats %>% inner_join(PONS_Demographics_mets_pats)

Lung_pats %>% group_by(chemo_class) %>% count() %>% rename("den"="n") %>%
  inner_join(Lung_pats %>% inner_join(PONS_Demographics_temp %>% select(patid) %>% distinct()) %>% group_by(chemo_class) %>% count() %>% rename("num"="n")) %>%
  mutate(perc=num/den) %>% ungroup() %>% mutate(relative_perc=perc/sum(perc))


#   chemo_class          den   num   perc relative_perc
#   <chr>              <int> <int>  <dbl>         <dbl>
# 1 Non-platinum Chemo  5422   485 0.0895         0.195
# 2 PD1/PDL1            6744   657 0.0974         0.213
# 3 PD1/PDL1_wChemo     2943   312 0.106          0.231
# 4 Platinum agent      8528   790 0.0926         0.202
# 5 Target              4459   323 0.0724         0.158
...
Intestinal_pats <- Intestinal_pats %>% inner_join(PONS_Demographics_mets_pats)


Intestinal_pats %>% group_by(chemo_class) %>% count() %>% rename("den"="n") %>%
  inner_join(Intestinal_pats %>% inner_join(PONS_Demographics_temp %>% select(patid) %>% distinct()) %>% group_by(chemo_class) %>% count() %>% rename("num"="n")) %>%
  mutate(perc=num/den) %>% ungroup() %>% mutate(relative_perc=perc/sum(perc))

#   chemo_class          den   num   perc relative_perc
# 1 Non-platinum Chemo  8947   589 0.0658         0.160
# 2 PD1/PDL1             630    61 0.0968         0.235
# 3 PD1/PDL1_wChemo      368    37 0.101          0.244
# 4 Platinum agent      6532   419 0.0641         0.156
# 5 Target              5112   433 0.0847         0.206
...
Pancreatic_pats <- Pancreatic_pats %>% inner_join(PONS_Demographics_mets_pats)

Pancreatic_pats %>% group_by(chemo_class) %>% count() %>% rename("den"="n") %>%
  inner_join(Pancreatic_pats %>% inner_join(PONS_Demographics_temp %>% select(patid) %>% distinct()) %>% group_by(chemo_class) %>% count() %>% rename("num"="n")) %>%
  mutate(perc=num/den) %>% ungroup() %>% mutate(relative_perc=perc/sum(perc))
 

#   chemo_class          den   num  perc relative_perc
# 1 Non-platinum Chemo  2778   385 0.139         0.178
# 2 PD1/PDL1             100    18 0.18          0.231
# 3 PD1/PDL1_wChemo       68    14 0.206         0.264
# 4 Platinum agent      1708   244 0.143         0.183
# 5 Target               338    38 0.112         0.144
