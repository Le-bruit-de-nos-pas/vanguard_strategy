...
# Pre-analytical stage Data Transformation
...
```{r Reshape of DIA (T2D) and OBE (Obesity) drug tables}
...
```{r Reshape of DIA (T2D) and OBE (Obesity) stocks tables}
...
```{r Reshape of DIA (T2D) and OBE (Obesity) doses tables}
...
## Initial Data and Methodology Analysis
```{r weights}
...
```{r T2D/Obesity diagnosed pts Age & Gender}
...
```{r Diagnosed, Treated population topline figures}
...
```{r Diagnosed population by source type}
...
```{r Lines of therapy}
# Using Mark's aggregated tables
...
```{r T2D/Obesity treated pts Age & Gender}
...
```{r DIA and Obesity patient vectors}
...
## Flows
```{r Flows Aux tables - long format}
...
```{r Patient distribution by number of flows - Last 12m}
...
```{r Inflows vs Outflows by Therapy class}
# period: last 12 months
...
```{r Switch Matrix - last 12 months}
...
## Drugs
```{r Nr lines of therapy vs time treated}
# Last 12 months
...
```{r Nr lines of therapy over time by stock}
...
```{r Nr lines of therapy by age and stock}
...
```{r nr pts on GLP1 injectable - month60}
...
## Architecture
```{r Architecture Stocks (m60) & Flows (last 12m) - interface data table}
...
## Deep Dives
```{r Profile of SGLT2 & DPP4 specific switchers}
...
```{r Insulin ouflowers - time on Insulin Rx}
