...
# ------------
# Insulins Novo vs Others ---------
OPTUMSES_MTHLY_LU_NDC_18062024 <- fread("DIA Analysis Results 1.1/OPTUMSES_MTHLY_LU_NDC_18062024.txt", sep=",",  colClasses = "character")

Insulin_NDCs <- OPTUMSES_MTHLY_LU_NDC_18062024 %>% filter(grepl("TRESIBA", BRND_NM)|
                                            grepl("LEVEMIR", BRND_NM)|
                                            grepl("FIASP", BRND_NM)|
                                            grepl("NOVOLOG", BRND_NM)|
                                            grepl("NOVOLIN", BRND_NM)|
                                            grepl("XULTOPHY", BRND_NM)|
                                            grepl("INSULATARD", BRND_NM)|
                                            grepl("MIXTARD", BRND_NM)|
                                            grepl("MIXTARD", BRND_NM)|
                                            grepl("MIXTARD", BRND_NM)|
                                            grepl("MIXTARD", BRND_NM)|
                                            grepl("MIXTARD", BRND_NM)|
                                            grepl("MIXTARD", BRND_NM)|
                                            grepl("MIXTARD", BRND_NM)) %>% select(NDC) 


DANU_Optum_Drugs <- fread("DIA Analysis Results 1.1/DANU Optum Drugs.txt", sep="\t", colClasses = "character")
DANU_Optum_Drugs <- DANU_Optum_Drugs %>% select(drug_id, ndc) %>% 
  inner_join(Insulin_NDCs, by=c("ndc"="NDC"))

Dia_US_Doses <- fread("DIA Doses 1.1/DIA Doses.txt", colClasses = "character")
range(Dia_US_Doses$from_dt)

Dia_US_Doses <- Dia_US_Doses %>% filter(from_dt>"2021-06-30") %>% filter(grepl("Insulin", drug_group))
sum(as.numeric(Dia_US_Doses$weight)*as.numeric(Dia_US_Doses$dayssup)) 

Dia_US_Doses_v2 <- Dia_US_Doses %>% inner_join(DANU_Optum_Drugs)
sum(as.numeric(Dia_US_Doses_v2$weight)*as.numeric(Dia_US_Doses_v2$dayssup)) 

# --------

# Compare Injuctables GLP1 and Insulin  Flows Last 12 months ----------------------------
DANU_Ingredients <- fread("DIA Analysis Results 1.1/DANU Ingredients.txt", integer64 = "character", stringsAsFactors = F)
...
DIA_Flows_Aux_Long %>% filter(!grepl("53", d1) & grepl("53", d2)) %>% 
  group_by(s1) %>% summarise(n=sum(weight)/1612801)
...
DIA_Flows_Aux_Long %>% filter(grepl("53", d1) & !grepl("53", d2)) %>% 
  group_by(s2) %>% summarise(n=sum(weight)/962546.2)
...
DIA_Flows_Aux_Long %>% filter(!grepl("49", d1) & grepl("49", d2)) %>% 
  group_by(s1) %>% summarise(n=sum(weight)/1160245)
...
DIA_Flows_Aux_Long %>% filter(grepl("49", d1) & !grepl("49", d2)) %>% 
  group_by(s2) %>% summarise(n=sum(weight)/825307.2)
...
DIA_Flows_Aux_Long %>% filter(!grepl("51", d1) & grepl("51", d2)) %>% 
  group_by(s1) %>% summarise(n=sum(weight)/447734.3)
...
DIA_Flows_Aux_Long %>% filter(grepl("51", d1) & !grepl("51", d2)) %>% 
  group_by(s2) %>% summarise(n=sum(weight)/533241.6)
...
DIA_Flows_Aux_Long %>% filter(!grepl("I", s1) & grepl("I", s2)) %>% 
  group_by(s1) %>% summarise(n=sum(weight)/4336832)
...
DIA_Flows_Aux_Long %>% filter(grepl("I", s1) & !grepl("I", s2)) %>% 
  group_by(s2) %>% summarise(n=sum(weight)/4314184)
...
# --------------
# DANU DIA GLP1 persistency by BMI group and N comorbidities WITH Number of T2D Drugs ------
DANU_Ingredients <- fread("DIA Analysis Results 1.1/DANU Ingredients.txt", integer64 = "character", stringsAsFactors = F)
DANU_Ingredients <- DANU_Ingredients %>%  separate(drug_id, c('class', 'molecule'))
DANU_Ingredients <- DANU_Ingredients %>% select(molecule, drug_group)
names(DANU_Ingredients)[1] <- "Drugs"
...
DIA_Drug_Histories <- fread("DIA Analysis Results 1.1/DIA Drug Histories.txt")
DIA_Drug_Histories <- gather(DIA_Drug_Histories, Month, Drugs, month1:month60, factor_key=TRUE)
DIA_Drug_Histories <- DIA_Drug_Histories %>% select(patient, Drugs) %>% distinct() %>% filter(Drugs!="-")
DIA_Drug_Histories <- separate_rows(DIA_Drug_Histories, Drugs, sep = ",", convert=T)
DIA_Drug_Histories <- DIA_Drug_Histories %>% distinct()
DIA_Drug_Histories <- DIA_Drug_Histories %>% group_by(patient) %>% count() %>% rename("N_drugs"="n")

AllDIA_episodes <- AllDIA_episodes %>% left_join(DIA_Drug_Histories)
...
summary(lm(Total_months_ON ~ HbA1c, data=AllDIA_episodes))
summary(lm(Total_months_ON ~ N_comorbs, data=AllDIA_episodes))
summary(lm(Total_months_ON ~ N_drugs, data=AllDIA_episodes))

summary(lm(Total_months_ON ~ BMI + HbA1c + N_comorbs, data=AllDIA_episodes))
summary(lm(Total_months_ON ~ BMI + HbA1c + N_comorbs + N_drugs, data=AllDIA_episodes))

summary(lm(Total_months_ON ~ HbA1c  + N_drugs, data=AllDIA_episodes))


AllDIA_episodes %>% ungroup() %>%
  ggplot(aes(N_drugs, Total_months_ON)) + geom_smooth(method="loess")
...
AllDIA_episodes %>% ungroup() %>%
  filter(N_drugs<13) %>%
  ggplot(aes(N_drugs, Total_months_ON)) + geom_smooth(method="gam")  +
  coord_cartesian(xlim=c(1,13), ylim=c(0,25)) +
  theme_minimal() +
  xlab("# T2D Drugs") + ylab("Total Months ON GLP1")
...
# DANU Utilizations Using OLD data ----------- 
DANU_Drug_Utilizations <- fread("DANU Utilizations/DANU Drug Utilizations.txt")
DANU_Utilizations <- fread("DANU Utilizations/DANU Utilizations.txt")

DANU_Utilizations <- DANU_Utilizations %>% select(patid, drug_ingredients)


DANU_Ingredients <- fread("DIA Analysis Results 2.1/DANU Ingredients.txt", integer64 = "character", stringsAsFactors = F)
DANU_Ingredients <- DANU_Ingredients %>%  separate(drug_id, c('class', 'molecule'))
DANU_Ingredients <- DANU_Ingredients %>% select(molecule, drug_group)
names(DANU_Ingredients)[1] <- "Drugs"
DANU_Ingredients$Drugs <- as.numeric(DANU_Ingredients$Drugs)
...
DIA_Drug_Histories <- fread("DIA Analysis Results 2.1/DIA Drug Histories.txt")
DIA_Drug_Histories <- gather(DIA_Drug_Histories, Month, Drugs, month1:month60, factor_key=TRUE)
DIA_Drug_Histories <- DIA_Drug_Histories %>%  mutate(Month=parse_number(as.character(Month)))
DIA_Drug_Histories <- DIA_Drug_Histories %>% mutate(ON=ifelse(grepl(string_GLP1, Drugs),1,0))
...
AllDIA_episodes %>% left_join(DANU_Utilizations, by=c("patient"="patid")) %>%
  filter(drug_ingredients <= 50) %>%
  ggplot(aes(drug_ingredients, Total_months_ON )) +
  geom_smooth(fill="#11576F", colour="black") +
  coord_cartesian(ylim=c(0,18), xlim=c(0,50)) +
  theme_minimal() +
  xlab("Number of Different Molecules Tried") + ylab("Total Months ON GLP1")



DIA_Comorbidity_Inventories <- fread("DIA Comorbidity Inventories 4.3/DIA Comorbidity Inventories.txt")
...
Comorbs_df <- AllDIA_episodes %>% rename("patid"="patient") %>% select(patid) %>%
  left_join(CKD) %>%
  left_join(POS) %>%
  left_join(PAD) %>%
  left_join(SLEEPAPNEA) %>%
...
Comorbs_df[is.na(Comorbs_df)] <- 0


Comorbs_df %>% ungroup() %>% select(-weight) %>%
  left_join(AllDIA_episodes, by=c("patid"="patient")) %>%
  gather(Comorb, Yes, CKD:DISLIPIDEMIA) %>%
  group_by(Comorb, Yes) %>% summarise(n=mean(Total_months_ON)) %>%
  spread(key=Yes, value=n)


Comorbs_df %>% ungroup() %>% select(-weight) %>%
  left_join(AllDIA_episodes, by=c("patid"="patient")) %>%
  gather(Comorb, Yes, CKD:DISLIPIDEMIA) %>%
  left_join(DANU_Utilizations) %>%
  group_by(Comorb, Yes) %>% summarise(n=mean(drug_ingredients)) %>%
  spread(key=Yes, value=n)
...
DIA_Drug_Histories <- DIA_Drug_Histories %>%  select(patient, Drugs) %>% distinct()
DIA_Drug_Histories <- DIA_Drug_Histories %>% mutate(ON=ifelse(grepl(string_GLP1, Drugs),1,0))
DIA_Drug_Histories <- DIA_Drug_Histories %>% select(patient) %>% distinct() %>%
  left_join(DIA_Drug_Histories %>% filter(ON==1) %>% select(patient, ON) %>% distinct()) 

DIA_Drug_Histories <- DIA_Drug_Histories %>% mutate(ON=ifelse(is.na(ON),0,ON))                                                    


ignore <- DIA_Drug_Histories %>% rename("patid"="patient") %>% 
  left_join(CKD) %>%
  left_join(POS) %>%
  left_join(PAD) %>%
  left_join(SLEEPAPNEA) %>%
...
ignore %>%
  gather(Comorb, Yes, CKD:DISLIPIDEMIA) %>%
  group_by(Comorb, Yes) %>% summarise(n=mean(ON)) %>%
  spread(key=Yes, value=n)
