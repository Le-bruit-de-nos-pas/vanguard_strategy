...
sum(CKD_Drug_Histories$weight) # 

CKD_Box_Histories <- fread("CKD Box Histories.txt")
sum(CKD_Box_Histories$weight) # 
...
sum(CKD_Demographics$weight) # 

CKD_Demographics %>% filter(died=="N" | death_date>"2020-05-01" ) %>% summarise(n=sum(weight)) # 
...
CKD_Demographics %>% filter(died=="N" | death_date>"2020-05-01" ) %>%
  ...
  inner_join(CKD_Drug_Histories) %>%  summarise(n=sum(weight))  # 
...
FirstMRA %>% select(patient, weight) %>% distinct() %>% ungroup() %>% summarise(n=sum(as.numeric(weight)))  # 
